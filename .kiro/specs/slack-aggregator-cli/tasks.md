...
- [x] 12. テストスイートの修正とアップデート
  - [x] 12.1 ConfigurationManagerテストの修正
    - 新しいトークン形式（appToken + botToken）に対応したテストケースに更新
    - isValidSlackToken、isValidSlackAppToken、isValidSlackBotTokenメソッドのテストを追加
    - 旧形式（単一token）と新形式（appToken + botToken）の両方をサポートするテストケースを作成
    - エラーメッセージの期待値を新しい形式に更新
    - _要件: 1.1, 1.2, 2.1_

  - [x] 12.2 SlackClientテストの修正
    - コンストラクタのテストを新しいトークン形式（appToken, botToken）に対応
    - 認証エラーテストケースを両方のトークンタイプに対応
    - モックオブジェクトの設定を新しいトークン形式に更新
    - _要件: 3.1, 3.2, 3.3_

  - [x] 12.3 TeamManagerテストの修正
    - チーム初期化テストを新しいトークン形式に対応
    - トークン検証エラーのテストケースを更新
    - チーム設定の期待値を新しい形式（appToken, botToken）に更新
    - _要件: 1.2, 1.3, 1.4_

  - [x] 12.4 統合テストの修正
    - 基本フローテストの設定を新しいトークン形式に更新
    - エラーシナリオテストの期待値を新しいエラーメッセージに対応
    - example-validation.testの検証ロジックを新しい.env.json.example形式に対応
    - _要件: 1.1-1.4, 2.1-2.4, 6.1, 6.2_

  - [x] 12.5 テストスイート全体の実行と検証
    - 全テストケースが正常に実行されることを確認
    - テストカバレッジが適切に維持されていることを確認
    - 新しいトークン形式での動作が正しくテストされていることを確認
    - _要件: 5.1, 5.3_

- [x] 13. 統合テストの修正と検証機能の実装
  - [x] 13.1 ConfigurationManagerの検証機能強化
    - loadConfig()メソッドで無効なトークン形式を検出してエラーを投げる機能を実装
    - 無効なチャンネルID形式を検出してエラーを投げる機能を実装
    - 空のチャンネル配列や欠損チャンネルプロパティの検証機能を実装
    - 設定ファイル不存在時の適切なエラー処理を実装
    - 無効なJSON形式の検出とエラー処理を実装
    - teams セクション不存在や空の teams セクションの検証機能を実装
    - _要件: 1.1, 1.2, 1.3, 2.1, 6.2_
  - [x] 13.2 ConfigurationManagerのファイル存在チェック修正
    - configFileExists()メソッドが正しいファイルパスをチェックするよう修正
    - テスト環境で適切なファイルパスが使用されるよう修正
    - _要件: 1.1, 5.1, 5.3_
  - [x] 13.3 loadConfigSafely()メソッドの修正
    - エラー発生時に適切にnullを返すよう修正
    - エラーログの出力を適切に行うよう修正
    - _要件: 1.3, 6.2_
  - [x] 13.4 error-scenarios.test.jsの設定パス修正
    - 全てのConfigurationManagerインスタンスで正しいテストファイルパス（testConfigPath）を使用するよう修正
    - 各テストが独立して実行できるよう設定ファイルパスを適切に設定
    - _要件: 5.1, 5.3_
  - [x] 13.5 全テストスイートの最終検証
    - 全テストケースが正常に実行されることを確認
    - テストカバレッジが適切に維持されていることを確認
